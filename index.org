...
I need to figure out how to store items. An item can be on the floor or carried by a colonist. That means its =pos= is of type =pos: Position | Colonist=. For pathfinding I also have an =occupied= map from position to =Furniture | Item | null=. So the transitions will be:

1. Item created on the ground (e.g. berries): create an =item=, ensure =occupied[item.pos]= is =null=, and set =occupied[item.pos]= to =item=.
1. Item is consumed on the ground (e.g. crafting): set =occupied[item.pos]= to =null=, set =item.pos= to =null=, and destroy =item=.
1. Colonist picks up item: ensure =colonist.inventory= is =null=, set =occupied[item.pos]= to null, set =item.pos= to the colonist, and set =colonist.inventory= to =item=.
1. Colonist drops item: ensure =occupied[newpos]= is =null=, set =occupied[newpos]= to =item=, set =item.pos= to =newpos=, and set =colonist.inventory= to =null=.
...
TODO: I've worked this out on paper but haven't implemented or tested it.
...
3. The current time is past =job.timeCompleted=. Destroy the input objects. Create an output object, and put it into =colonist.inventory=. Set the item's =.pos= to the colonist. Set the colonist to find a path to =dest=. Set =stand.job= to null (this frees up the furniture to start another job). Set the job phase to /deliver/.
...
<x:footer>
  Created 26 Jun 2023; &#160;
  <!-- hhmts start -->Last modified: 16 Aug 2023<!-- hhmts end -->
</x:footer>
#+end_export
