...
Multi-tile overlap check algorithm: given a location in a room, I need to figure out whether furniture can be placed there. A helper function can calculate all the positions that the furniture uses up (stand position, input positions, and sprite positions). Then, for each position in the set:

1. check that it's in the room bounds
2. for each existing furniture in the same room, check that it's not already occupied by that furniture
...
<figure>
  ...
  <figcaption>When placing furniture, mark which tiles can't be built on in red</figcaption>
</figure>
#+end_export
...
<x:footer>
  Created 26 Jun 2023; &#160;
  <!-- hhmts start -->Last modified: 07 Aug 2023<!-- hhmts end -->
</x:footer>
#+end_export
